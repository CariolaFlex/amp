...
const ventasMensuales = [
  { mes: 'Dic', monto: 24500000 },
  { mes: 'Ene', monto: 31200000 },
  { mes: 'Feb', monto: 18900000 },
  { mes: 'Mar', monto: 42300000 },
  { mes: 'Abr', monto: 38700000 },
  { mes: 'May', monto: 26700000 },
];

const ETAPAS_COLORS: Record<string, string> = {
  prospeccion: '#6366f1',
  calificado: '#f59e0b',
  cotizacion_enviada: '#3b82f6',
  negociacion: '#8b5cf6',
  ganada: '#22c55e',
  perdida: '#ef4444',
};
...
export default function DashboardPage() {
  const facturacionMes = mockDtes.filter(d => d.estado !== 'anulado' && d.estado !== 'rechazado').reduce((s, d) => s + (d.total > 0 ? d.total : 0), 0);
  const dealsGanados = mockOportunidades.filter(o => o.etapa === 'ganada').length;
  const stockBajoMin = mockProductos.filter(p => p.estado === 'bajo_minimo' || p.estado === 'sin_stock').length;
  const cxcVencidas = mockCxC.filter(c => c.aging === '+90' || c.aging === '61-90').length;
  const dtesRechazados = mockDtes.filter(d => d.estado === 'rechazado');
  const pendienteAcuse = mockDtesProveedor.filter(d => d.estado === 'pendiente_acuse');

  const pipelineData = Object.entries(
    mockOportunidades.reduce((acc, op) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Constructora Los Andes SpA · {formatDate(new Date())}</p>
      </div>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <KpiCard
          ...
          value={formatCLP(facturacionMes)}
          sub="Mayo 2026"
          icon={DollarSign}
          trend={{ value: '+12% vs abril', up: true }}
        />
        <KpiCard
          title="Deals ganados"
          value={String(dealsGanados)}
          sub="Este mes"
          icon={CheckCircle2}
          trend={{ value: '+1 vs abril', up: true }}
        />
        <KpiCard
          title="Stock bajo mínimo"
          value={String(stockBajoMin)}
          sub="Productos críticos"
          icon={Package}
          variant={stockBajoMin > 0 ? 'warning' : 'default'}
        />
        <KpiCard
          title="CxC vencidas"
          value={String(cxcVencidas)}
          sub="+61 días sin cobrar"
          icon={Clock}
          variant={cxcVencidas > 0 ? 'danger' : 'default'}
        />
      </div>
      ...
      {(dtesRechazados.length > 0 || pendienteAcuse.length > 0) && (
        <div className="space-y-2">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Alertas críticas</h2>
          <div className="space-y-2">
            ...
            {pendienteAcuse.map(dp => {
              ...
              return (
                <div key={dp.id} className={`flex items-center gap-3 rounded-lg border px-4 py-3 ${
                  dias <= 1 ? 'border-destructive/30 bg-destructive/5' : 'border-warning/30 bg-warning/5'
                }`}>
                  ...
                  <Badge variant={dias <= 1 ? 'destructive' : 'warning'}>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
          ...
          <CardContent className="pt-4">
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={ventasMensuales} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                ...
                <Tooltip
                  formatter={(v: unknown) => [formatCLP(v as number), 'Ventas']}
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="pb-0">
            ...
            <p className="text-xs text-muted-foreground">{mockOportunidades.length} oportunidades</p>
          </CardHeader>
              ...
              <PieChart>
                ...
                <Tooltip
                  ...
                  formatter={(v: unknown, _: unknown, p: any) => [
                    `${v} deals · ${formatCLP(p?.payload?.monto ?? 0)}`, p?.payload?.name ?? '',
                  ]}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-semibold">Actividades recientes</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {[
              { icon: CheckCircle2, color: 'text-success', text: 'DTE #1024 aceptado por SII — Minera Atacama SA', sub: 'Hace 2 horas' },
              { icon: Users, color: 'text-primary', text: 'Oportunidad "Ampliación Hotel Boutique" marcada como ganada', sub: 'Hace 4 horas' },
              { icon: Package, color: 'text-warning', text: 'Stock bajo mínimo: Varilla de acero Ø20mm (85 mt disponibles)', sub: 'Hace 6 horas' },
              { icon: FileText, color: 'text-muted-foreground', text: 'Cotización enviada a Pesquera del Pacífico Ltda', sub: 'Ayer' },
            ].map((a, i) => (
