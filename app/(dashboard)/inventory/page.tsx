...
export default function InventoryPage() {
  ...
  const categorias = ['Todos', ...Array.from(new Set(mockProductos.map(p => p.categoria)))];
  const filtered = categoriaFilter === 'Todos' ? mockProductos : mockProductos.filter(p => p.categoria === categoriaFilter);

  const okCount = mockProductos.filter(p => p.estado === 'ok').length;
  const warnCount = mockProductos.filter(p => p.estado === 'bajo_minimo').length;
  const critCount = mockProductos.filter(p => p.estado === 'sin_stock').length;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{mockProductos.length} productos en catálogo</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="h-8 text-xs" asChild>
            <Link href="/inventory/movimientos">Movimientos</Link>
          </Button>
          <Button size="sm" className="h-8 text-xs"><Plus className="h-3.5 w-3.5 mr-1" />Nuevo producto</Button>
        </div>
      </div>
      ...
      <DataTable
        ...
        actions={() => (
          <>
            <DropdownMenuItem className="text-xs">Ver movimientos</DropdownMenuItem>
            <DropdownMenuItem className="text-xs">Ajustar stock</DropdownMenuItem>
